...
function Guard({children}:{children:React.ReactNode}) { const {session,loading,configured}=useAuth(); const router=useRouter(); useEffect(()=>{if(!loading&&configured&&!session)router.replace("/login");},[session,loading,configured,router]); if(loading)return <div className="min-h-screen grid place-items-center muted text-sm">Loading Apex Quant Trader...</div>; return <GatewayProvider><DashboardShell>{children}</DashboardShell></GatewayProvider>; }
