...
export default function Execution() {
  ...
  /* Subscribe/unsubscribe when selected engine changes */
  useEffect(() => {
    setExecutionMetricsEngine(selectedId);
    return () => setExecutionMetricsEngine(null);
  }, [selectedId, setExecutionMetricsEngine]);
  ...
  return (
    <div className="p-5 md:p-8 max-w-[1680px] mx-auto space-y-5 page-in">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="text-[10px] uppercase tracking-[.17em] muted">Private execution domain</div>
          <h1 className="text-3xl font-semibold mt-2 tracking-tight">My Execution</h1>
          <p className="muted mt-1 text-sm">
            Owner-scoped account, risk, trade, and broker execution telemetry from your installed engine.
          </p>
        </div>
        ...
        {!enginesLoading && engines.length > 1 && (
          <div className="relative">
            <select
              ...
              className="appearance-none panel pl-3 pr-8 py-2 text-sm cursor-pointer hover:border-white/20 transition-colors min-w-[200px]"
              style={{ borderRadius: "var(--radius-control)", background: "var(--surface-soft)" }}
            >
              {engines.map(e => (
                <option key={e.id} value={e.engine_id} style={{ background: "#0e1013" }}>
                  {e.device_name || e.engine_id}
                </option>
              ))}
            </select>
            <ChevronDown size={13} className="absolute right-3 top-1/2 -translate-y-1/2 muted pointer-events-none" />
          </div>
        )}
      ...
      {!enginesLoading && engines.length === 0 && (
        <div className="panel p-6 state-block">
