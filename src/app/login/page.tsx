...
export default function LoginPage() {
  ...
  async function signInGoogle() {
    ...
    if (!googleEnabled) return setMessage("Google sign-in is not enabled in the TradeRelay Supabase project yet.");
    setLoading(true); setMessage("");
  ...
  return <main className="login-shell">
    <aside className="login-panel-left">
      <Link href="/" className="flex items-center gap-3"><div className="w-9 h-9 rounded-md border border-[#3ddc97]/45 bg-[#052318] grid place-items-center mono text-[11px] font-bold">TR</div><div><div className="text-xs font-bold tracking-[.16em]">TRADERELAY</div><div className="text-[9px] uppercase tracking-[.12em] muted mt-0.5">Execution network</div></div></Link>
      ...
      <p className="text-[10px] muted leading-5 mt-8">By continuing you agree to TradeRelay&apos;s Terms and Privacy Policy.</p>
    </aside>
    <section className="login-panel-right"><div className="login-form">
      ...
      : magicSent ? <div className="panel p-4 flex gap-3"><CheckCircle size={17} className="text-[#3ddc97] shrink-0"/><div><div className="text-sm font-semibold">Magic link sent</div><div className="text-xs muted mt-1 leading-5">Open the link in your email to securely enter TradeRelay.</div><button onClick={reset} className="auth-link mt-3">Use a different email</button></div></div>
